...
public:
	...
	Hand_Demo_Warehouse_Onecamera():planned(false),moved(false),initialized(false),pnh_("~"),
	targets ({
		//"open",
		"chinese_number_0",
		"chinese_number_1",
	...
	})
	{
		...
		pose_pub = pnh.advertise<hand_demo::NamedRobotPose>("/named_robot_pose",1);

		bool randomize_= pnh.param<bool>("random", true);
		double image_interval_= pnh.param<double>("image_interval", 0.05);
		double wrist1_lower_limit_ = pnh.param<double>("wrist1_lower_limit", -0.698);
		double wrist1_upper_limit_ = pnh.param<double>("wrist1_upper_limit", 0.489);
		double wrist2_lower_limit_ = pnh.param<double>("wrist2_lower_limit", -0.489);
		double wrist2_upper_limit_ = pnh.param<double>("wrist2_upper_limit", 0.140);
  	...
  	psi = std::make_shared<moveit::planning_interface::PlanningSceneInterface>();
		...
		collision_detection::AllowedCollisionMatrix full_acm = get_allowedcollisionmatrix();
		...
		for (int i = 0; i < w1.rows(); i++)
		{
			for (int y = 0; y < w2.rows(); y++)
			{
				...
				for( auto& t : targets ){
					...
					collision_allowance(mgi,psi,full_acm);
					...
					collision_allowance(psi, full_acm);
				}
			}
		}
	}
	...
	void collision_allowance(moveit::planning_interface::MoveGroupInterfacePtr& mgi, moveit::planning_interface::PlanningSceneInterfacePtr& psi, collision_detection::AllowedCollisionMatrix& full_acm, planning_scene::PlanningScene& planning_scene)
	{
		...
		robot_state::RobotState current_state = mgi->getJointValueTarget();
	...
	void forbid_collision(moveit::planning_interface::PlanningSceneInterfacePtr& psi, collision_detection::AllowedCollisionMatrix& full_acm)
	{
		moveit_msgs::PlanningScene scene_msg;
		scene_msg.is_diff= true;
		scene_msg.robot_state.is_diff= true;
	...
	void One_camera_callback(const sensor_msgs::Image::ConstPtr &image_data)
	{
		...
		ros::Duration(image_interval).sleep();
	}
